...
\begin{enumerate}
    ...
    %, based on the assumptions that each feature has the same effect size, \textbf{after controlling for spatial decay.}
    ...
    \item The approach we developed allows to assess cumulative impacts of infrastructure on biodiversity, and can be broadly applied in environmental impact assessment and land use planning. All ZoI metrics are calculated before statistical analysis, making them computationally efficient and flexible, and can be computed in R and GRASS GIS through the \verb|oneimpact| R package. Although our example focuses on animal space use, the approach can be applied %to improve accuracy 
...
% paragraph mentioning the usage of that in other approaches, such as multi-species SDMs, conscape and other connectivity modeling approachs
