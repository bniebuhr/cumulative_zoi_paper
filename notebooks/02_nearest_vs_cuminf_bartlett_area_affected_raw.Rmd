...
nfeat <- c(1, 5, 10, 20, 30, 40, 50, 80, 100, 150, 200, 500) # number of features
res <- 100 # resolution
...
# ZoI values
zoi_vals <- c(250, 500, 1000, 2000, 5000, 10000)

# parameters
parms_df1 <- expand.grid(method = methods, n_features = nfeat,
                        centers = nc[1], width = wd) %>%  # regular, random, clumped5
  dplyr::filter(!(method == "regular" & width == 1500)) %>% 
  dplyr::filter(!(method == "random" & width == 1500)) # regular and random do not change with width
...
parms_df <- dplyr::bind_rows(parms_df1, parms_df2) %>% 
  dplyr::arrange(n_features, method, desc(width))
```
...
# raster with points
pts_all_rast_100 <- pts_100 %>% 
  purrr::map(~ .[[2]])

# vector with points, for plotting
pts_vect_list <- pts_coords %>% 
  purrr::map(sf::st_as_sf, coords = c(1:2), crs = terra::crs(pts_all_rast_100[[1]]))
for(i in 1:length(pts_vect_list)) {
  ind <- (i %% length(name))
  if(ind == 0) ind <- 6
  pts_vect_list[[i]]$spatial_dist <- name[ind]
  ind_int <- ceiling(i / length(name))
  pts_vect_list[[i]]$n <- nfeat[ind_int]
}
...
landscape <- st_bbox(c(xmin = 0, xmax = ext, ymax = 0, ymin = ext), crs = terra::crs(pts_all_rast_100[[1]])) %>% 
  st_as_sfc()
...
create_scenarios <- parms_df %>% 
  dplyr::rename(spatial_dist = method) %>% 
  dplyr::mutate(
    # mean isolation
    mean_isolation = c(NA, NA, NA, NA, NA, NA, 
                       map_dbl(pts_coords[7:length(pts_coords)], 
                               oneimpact::mean_isolation, n_rand = 140, ext = c(0, ext))),
...
maps_cuminf <- purrr::map2(scenarios_dist$points_100, scenarios_dist$zoi, function(x, y) {
  print(paste("running zoi", y, "..."))
...
for(i in 1:length(maps_cuminf)) terra::writeRaster(maps_cuminf[[i]],
                                                   filename = sprintf("../data/test/n_cuminf%03d.tif", i), 
                                                   overwrite = TRUE)
```

```{r load_cuminf, eval=TRUE, echo=FALSE}
maps_cuminf <- list()
cuminf_rasts_full <- list.files("../data/test/", pattern = "^n_cuminf", full.names = T)
...
maps_nearestinf <- purrr::map2(scenarios_dist$points_100, scenarios_dist$zoi, function(x,y) {
  print(paste("running zoi", y, "..."))
...
for(i in 1:length(maps_nearestinf)) terra::writeRaster(maps_nearestinf[[i]],
                                                       filename = sprintf("../data/test/n_nearest%03d.tif", i),
                                                       overwrite = TRUE)
```

```{r load_nearest, eval=TRUE, echo=FALSE}
maps_nearestinf <- list()
nearest_rasts_full <- list.files("../data/test/", pattern = "^n_nearest", full.names = T)
...
scenarios_dist <- scenarios_dist %>% 
  dplyr::mutate(
    ...
    corr = purrr::map2(rast_nearest, rast_cuminf, function(x, y)
      try(cor(terra::values(x), terra::values(y))))) %>% 
  tidyr::unnest(corr) %>% 
  dplyr::mutate(spatial_dist = ifelse(spatial_dist == "mobsim", 
                                      paste0("clumped", centers, "_", as.integer(width/ext*100)),
                                      as.character(spatial_dist)),
                spatial_dist = factor(spatial_dist, levels = name))
  # dplyr::mutate(labels = paste(LETTERS[1:length(name)], name[c(1,2,3,4,5,6)], sep = ": "))
...
scenarios_dist %>% 
  ...
  geom_vline(aes(xintercept = width*1.96), linetype = 2, color = "darkgrey") +
  geom_point() +
  geom_line() +
  ylim(00, 1) +
...
which_scenarios <- c(6) - 1
which_n <- which(nfeat == 10) - 1
indices1 <- which_scenarios*length(zoi_vals) + which_n*length(name)*length(zoi_vals) + 1:length(zoi_vals)

to_viz1 <- do.call(c, scenarios_dist$rast_nearest[indices1])
names(to_viz1) <- paste(rep("nearest", nlyr(to_viz1)), zoi_vals[1:length(zoi_vals)],
                       sep = ": ")
plot(to_viz1, nc = 3)
```
...
to_viz1 <- do.call(c, scenarios_dist$rast_cuminf[indices1])
names(to_viz1) <- paste(rep("cumulative", nlyr(to_viz1)), zoi_vals[1:length(zoi_vals)],
                       sep = ": ")
plot(to_viz1, nc = 3)
```
