...
abstract: | 
  ...
  multiple infrastructure for types of infrastructure and spatial variables that are represented
  as lines (e.g. roads, power lines) or polygons (e.g. dams, mining sites). This complements the derivation
  for point-type infrastructure as presented in Niebuhr et al. *Estimating the cumulative impacts and 
...
In the main text, we derived the estimation of the cumulative influence of multiple infrastructure under different assumptions, but only did so for infrastructure that can be represented as points, such as houses, cabins, or wind turbines. Here we complement it by deriving similar equations for linear infrastructure (e.g. roads, trails, power lines) and infrastructure or landscape variables represented by polygons (mining sites, hydro dams, deforestation areas or polygons outlining specific land cover and land use types).
...
to influence processes in a given location. In such a way, their influence is similar to that of a point. As a consequence, equation 3 from the main text keeps valid to represent the influence of the nearest feature:
...
The only difference here regards how the cumulative influence function $\phi_{cum}$ is defined -- as a sum or an integral over 1 or 2 dimensions. 

# Calculation of the cumulative influence

In theory, the calculation of the cumulative influence function $\phi_{cum}$ for a given study area $\Omega$ requires the computation of eq. 4 from the main text or eq. \ref{eqn:HSFcuminf}, 
which might be 
difficult and computationally challenging for large areas with many infrastructure features spread non-regularly in space. To overcome that limitation, we implemented this calculation by constructing filters (or weighing matrices; see Miguet et al., 2017) that follow an assumed decaying function (see the functions in Fig. 1 of the main text for examples) and used them on moving window/neighborhood analyses over the input infrastructure layers. This is similar to the approaches presented by Miguet et al. (2017) and Gilleland (2013). We do not show it here, but, if the filter is set appropriately to include most of the area within which the influence function is positive (e.g. larger than 0.05 or 0.01), the result of applying eq. 4 or eq. \ref{eqn:HSFcuminf} presents a correlation $> 0.99$ with the 
result of the neighborhood analysis.
