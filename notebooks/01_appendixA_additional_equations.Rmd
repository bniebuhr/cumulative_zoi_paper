---
title: "Appendix A"
subtitle: "Defining and deriving the zone of influence for multiple infrastructure features"
author: ""
date: ""
geometry: margin=2cm
abstract: | 
  In this document, we provide additional details around the definition of the zone of influence (ZoI). First, we define the ZoI $\phi$ and present examples of ZoI functions with different shapes, parameterized with the ZoI radius. Second, we derive the ZoI of the nearest feature and the cumulative ZoI of multiple features of an infrastructure for types of infrastructure and spatial variables that are represented as lines (e.g. roads, power lines) or polygons (e.g. dams, mining sites). This complements the derivation for point-type infrastructure as presented in the main text of Niebuhr et al. *Estimating the cumulative impact and zone of influence of anthropogenic infrastructure on biodiversity.*
...
assumptions, but only did so for infrastructure that can be represented as points, 
...
impact of a given infrastructure feature decreases with distance. Formally, the 
ZoI $\phi = f(d, r)$ is any decay function that has a maximum value 
1 where infrastructure is located, decreases towards zero as the Euclidean 
...
$$
\phi_{threshold}(d_{i_k}, r_k) = 
    \begin{cases}
      1 & \text{if $d_{i_k} < r_k$} \\
      0 & \text{if $d_{i_k} \ge r_k$}
    \end{cases}
$$

where $d_{i_k}$ is the distance to the infrastructure $i$ of type $k$ and $r_k$ 
is the radius of the ZoI for infrastructure of type $k$. Then, $\phi_{threshold}$ 
represents a constant impact of each infrastructure feature until $d = r$ and is 
similar to what has generally been used when buffer zones are created around 
infrastructure to calculate derived measures.

Fig. A\ref{fig:plot_threshold} shows a plot of $\phi_{threshold}$ with $r = 10$ km.

```{r plot_threshold, echo=FALSE, message=FALSE, error=FALSE, warning=FALSE, fig.cap="Illustration of a threshold ZoI function with radius = 10 km."}
library(oneimpact)
library(ggplot2)

oneimpact::plot_zoi1d(points = 0, zoi_radius = 10, fun = threshold_decay, 
                      range_plot = c(-20, 20)) +
  labs(x = "Distance (km)")
```
...
(Harris, 1978), corresponds to 1 at the infrastructure location and decreases 
...
$$
\phi_{linear}(d_{i_k}, r_k) = 
    \begin{cases}
      1 - \frac{d_{i_k}}{r_k} & \text{if $d_{i_k} < r_k$} \\
      0 & \text{if $d_{i_k} \ge r_k$}
    \end{cases}
$$

where $d_{i_k}$ is the distance to the infrastructure $i$ of type $k$ and $r_k$ 
is the radius of the ZoI for infrastructure of type $k$.

Fig. A\ref{fig:plot_linear} shows a plot of $\phi_{linear}$ with $r = 10$ km.

```{r plot_linear, echo=FALSE, message=FALSE, error=FALSE, warning=FALSE, fig.cap="Illustration of a linear decaying ZoI function with radius = 10 km."}
oneimpact::plot_zoi1d(points = 0, zoi_radius = 10, fun = linear_decay, 
                      range_plot = c(-20, 20)) +
  labs(x = "Distance (km)")
```
...
functions. For both we set $\phi_{limit} = 0.05$ for all infrastructure.
...
$$
\phi_{exp}(d_{i_k}, r_k, \phi_{limit_k}) = \exp(-\lambda_{r_k} d_{i_k})
$$

where $d_{i_k}$ is the distance to the infrastructure $i$ of type $k$ and the 
...
```{r plot_exponential, echo=FALSE, message=FALSE, error=FALSE, warning=FALSE, fig.cap="Illustration of an exponential decaying ZoI function with radius = 10 km."}
oneimpact::plot_zoi1d(points = 0, zoi_radius = 10, fun = exp_decay, 
                      range_plot = c(-20, 20)) +
  labs(x = "Distance (km)")
```
...
$$
\phi_{Gauss}(d_{i_k}, r_k, \phi_{limit_k}) = \exp(-\lambda_{r_k} d_{i_k}^2)
$$
...
```{r plot_gaussian, echo=FALSE, message=FALSE, error=FALSE, warning=FALSE, fig.cap="Illustration of a Gaussian decaying ZoI function with radius = 10 km."}
oneimpact::plot_zoi1d(points = 0, zoi_radius = 10, fun = gaussian_decay, 
                      range_plot = c(-20, 20)) +
  labs(x = "Distance (km)")
```
...
# Derivation of zones of influence for multiple features of a type of infrastructure
...
infrastructure (or other spatiotemporal variables) and $\beta_k$ represents the 
effect size of the infrastructure of type $k$.

## ZoI metrics

As shown in the main text, when multiple features of a type of infrastructure are
...
We assume that in the landscape there are $n_k$ features of the same type of 
infrastructure $k$, and let the influence of the feature $i$ of an infrastructure 
...
\label{eqn:HSFtermLine}
    I_k = \beta_k X_k = \int_{L} \beta_{k}(l) \ \phi_{k}(l; d_{i_k}, r_k) dl
\end{equation}
...
\label{eqn:HSFtermPolygon}
    I_k = \beta_k X_k = \iint_{\Omega} \beta_{k}(x,y) \ \phi_{k}(x, y; d_{i_k}, r_k) \ dx \ dy
\end{equation}
...
infrastructure, one generally considers $\beta_i = 0$ for all $i > 1$ (where the 
...
the influence of the nearest feature for linear and area variables:
...
To derive the cumulative ZoI of multiple features of an infrastructure type, 
for point-like infrastructure we assumed that all features exerted the same influence, 
so that $\beta_1 =\beta_2 = ... = \beta_k = \beta$. If we do similar assumptions 
...
is defined -- as a summation for point infrastructure (eq. 5 in the main text) or 
as an integral over 1 or 2 dimensions for linear and area infrastructure (eq. 
\ref{eqn:HSFcumzoi}). 
...
infrastructure features spread non-regularly in space. To overcome that limitation, 
...
input infrastructure layers (see Appendix D for an illustration of how this 
approach is applied using the R package `oneimpact`). This is analogous to the 
