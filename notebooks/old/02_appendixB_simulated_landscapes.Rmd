---
title: "Appendix B"
subtitle: "Comparing the the influence of the nearest feature with the cumulative influence of multiple features"
author: "Bernardo Niebuhr, Bram van Moorter, Manuela Panzacchi"
date: "`r format(Sys.time(), '%d %B, %Y')`"
geometry: margin=2cm
abstract: | 
  In this document, we aim to illustrate in which conditions the influence of the nearest feature 
  ($\phi_{nearest}$) and the cumulative influence of multiple features ($\phi_{cum}$) diverge or converge.
  First we explore it by simulating landscapes with different spatial configurations of infrastructure features.
  Second, we calculate these measures and compare them for tourist-related infrastructure in the Hardangervidda
  wild reindeer area in Southern Norway. Understanding how these influence measures change across
  landscapes is important to aid the interpretation of their effects on ecological contexts. This document complements
  the arguments presented in Niebuhr et al. *Estimating the cumulative impacts and the zone of influence from 
...
There are several dimensions that underlie the assessment of the impacts of anthropogenic infrastructure on wildlife.
...
there are multiple infrastructure and vectors of landscape modification. In the main text of this manuscript, 
we argue that the cumulative influence measure ($\phi_{cum}$) -- proportional to the spatial density of infrastructure features -- 
might better represent the cumulative effect of multiple features in the landscape than considering only the distance from 
the nearest feature to a given location ($\phi_{nearest}$). How similar they are and whether one can distinguish between
these two measures, however, might vary depending on the number of features in the landscape, 
how these features are distributed in space, and what is the ZoI of each of those features.

In this document we aim to assess in which conditions $\phi_{nearest}$ and $\phi_{cum}$ converge and diverge, to allow a better 
interpretation of their effects on biological response variables in ecological contexts. 
We first simulate landscapes with point-type infrastructure spread following different patterns, calculate both influence measures, 
considering multiple scales/ZoI, and assess when and how these variables might represent different sources of spatial variation.
Second, we do the same for a real landscape, the Hardangervidda wild reindeer area in Southern Norway, 
which is the stage for the the habitat selection analyses presented in the main text and in Appendix C.

# Influence measures in simulated landscapes

## Simulating landscapes

We start by simulating landscapes using point-type infrastructure. They could represent the spatial location of houses, 
cabins, or wind turbines, for example. We set 30x30 km^2^ landscapes and simulate points following different spatial patterns:
regular, random, and clumped distribution of points. For the clumped distribution, we set four landscapes by varying the number
of clusters and the size of the clusters: point features 
are spread on either 5 or only 1 focal cluster, and we choose two possible radii of these clusters, corresponding to one more and 
one less clumped point distribution (radius = 5% [1.5 km] and 15% [4.5 km] of the extent of the landscape)[^1]. In such a way,
we aim to represent a gradient of spatial clustering of the simulated point features, to be able to evaluate how the influence 
measures vary within this gradient. To keep the interpretation simple, we fix the total number of features in the landscape to $n_k = 100$.
Yet, we also performed simulations with different numbers of features ($n_k = 10$) and the results are qualitatively similar.
<!-- To do this we'll use a few functions designed within the R package `oneimpact`. We also load spatial packages it depends on  -->
<!-- and other packages for data manipulation and plotting. -->

[^1]: To be precise, these values correspond to the standard deviation of the distance from points to the center of the cluster,
simulated using a bivariate Gaussian distribution. To ease the interpretation, however, we call them radii of the clusters.

The landscapes are simulated using the function `set_points()`, from the `oneimpact` package. 
Fig. B1 shows six example landscapes simulated in this gradient of clustering.
...
## Calculating the influence measures

For each of the simulated landscapes, we calculate both $\phi_{nearest}$ and $\phi_{cum}$ with ZoI
values varying from ~ 0.06% to 40% of the landscape extent (i.e., from 20 m to 12 km, 
given our landscapes are squares of 30 km extent). 
For the comparison presented here, we computed both measures of influence 
using a linear decay influence function (see Fig. 1 in the main text),
a tent-shaped function (also called "Bartlett" filter; Harris, 1978) 
that corresponds to 1 at the infrastructure location and decreases 
linearly with the distance from it, until it reaches zero at a given distance,
defined here as the ZoI of the infrastructure. 
Formally, the Bartlett (or linear decay) influence function distance $\phi_{Bartlett}$ is defined 
for an infrastructure of type $k$ as:

$$
\phi_{Bartlett}(d_{i_k}, ZoI_k) = 
    \begin{cases}
      1 - \frac{d_{i_k}}{ZoI_k} & \text{if $d_{i_k} \le ZoI_k$} \\
      0 & \text{if $d_{i_k} > ZoI_k$}
    \end{cases}
$$

where $d_{i_k}$ is the Euclidean distance from each pixel to the infrastructure feature $i$.
We chose the linear decay for illustration because the ZoI can be clearly defined
(i.e., the distance above which the influence of the infrastructure becomes zero).
...
## Visualizing the influence measures

To visualize qualitatively how different are $\phi_{nearest}$ and $\phi_{cum}$, we plot them
for different spatial configuration of features, considering different ZoI. 
We see in Figures B2, B3, and B4
the two measures for ZoI = 1.6%, 5%, and 10% of the landscape extent (500, 1500, and 3000 m). 
When ZoI is small compared to the landscape extent, the two influence measures are similar and,
in the computation of $\phi_{cum}$, there is interaction between the influence of multiple features 
only for very clustered feature distributions (Fig. B2). 
As the ZoI increases, however, going beyond the 
minimum distance between features, the influence from different features interact more in the calculation of
$\phi_{cum}$ and the two influence measures diverge (Fig. B3 and B4). 
Notice that the values in the color scale change between influence measures for the different
conditions of feature spatial configuration in Figs. B2-B4.

```{r viz_several_500, eval=TRUE, echo=FALSE, fig.cap="Illustration of the influence of the nearest feature and the cumulative influence of features for 4 landscapes in a gradient of clustering of the point infrastructure, for ZoI = 1.66\\% of the landscape extent (500 m)."}
...
```{r viz_several_1500, eval=TRUE, echo=FALSE, fig.cap="Illustration of the influence of the nearest feature and the cumulative influence of features for 4 landscapes in a gradient of clustering of the point infrastructure, for ZoI = 5\\% of the landscape extent (1500 m)."}
which_landscapes <- c(0:2, 5)
...
```{r viz_several_3000, eval=TRUE, echo=FALSE, fig.cap="Illustration of the influence of the nearest feature and the cumulative influence of features for 4 landscapes in a gradient of clustering of the point infrastructure, for ZoI = 10\\% of the landscape extent (3000 m)."}
which_landscapes <- c(0:2, 5)
...
## Correlation and variation in the influence measures

To understand quantitatively how similar are $\phi_{nearest}$ and $\phi_{cum}$ in different
conditions, we calculated the Pearson correlation coefficient between them, pixel-by-pixel. 
We also calculated the coefficient of variation (CV, the ratio between the standard deviation
and the mean) for each measure for each landscape, as an ancillary quantity to interpret their correlation.

As seen in the plots above (Figs. B2-B4), when $2 \cdot ZoI$ is smaller than the minimum distance between features, 
the influence of different features does not overlap and $\phi_{nearest}$ is similar to $\phi_{cum}$
(correlation = 1; Fig. B5). This is easily noticed for the regular distribution of points, in which the relative
minimum distance between points is high (= 10% of the landscape or 3000 m; Fig. B2 and B3, Fig. B5A), 
but less evident in the other spatial configuration of points, since the minimum distance between
points if very small (Fig. B5B-F). As the ZoI increases, the two influence measures diverge.
Because $\phi_{nearest}$ does not account for cumulative effects, as the ZoI increases its CV decreases 
faster than the CV of $\phi_{cum}$, what explains why the two measures diverge (Fig. B6).
This goes on until a given ZoI for which the gradient represented by 
$\phi_{nearest}$ starts to get similar to $\phi_{cum}$ again[^2], as shown in Fig. B5. 
This is mostly evident for more clustered distributions of features (Fig. B5D-F). 
The point of inflection is related to the size of the clusters of features.
For ZoI values larger than the cluster size, the variation in $\phi_{nearest}$ 
within the clusters vary little compared to their values outside the cluster (Fig. B6), 
and the cluster of features starts to act as a ``super-feature" 
(e.g. groups of houses or wind turbines behave as urban areas or wind parks, respectively).

[^2]: Even though the correlation between $\phi_{nearest}$ and $\phi_{cum}$ increases for
large ZoI and clustered distributions of features, the absolute scale of each influence measure is
different -- see the values in the color scale in Figs. B2-B4.

```{r correlations, eval=TRUE, echo=FALSE, results='hide', fig.cap="Pixel-to-pixel correlation between the influence of the nearest feature and the cumulative influence of multiple features as the ZoI increases, for the different spatial configurations of point infrastructure. The ZoI was varied from 0.06\\% to 40\\% of the landscape extent (i.e., from 20 m to 12 km, given our landscapes are squares of 30 km extent)."}
# plot
...
It is important to remark that how clustered a given set of infrastructure features appear depends on the
extent of the study area. As an example, if the biological response is measured and assessed in a 
study area that comprises 2 km around a wind farm whose turbines are spread in a few mountain tops, 
the distribution of wind turbines might look random or somehow aggregated. However, if the study area 
comprises a much bigger area and the biological response is expected to respond at larger extents, 
and the wind farm is only located in a small part of that area, their distribution might appear very clumped. 

```{r cv, eval=TRUE, echo=FALSE, results='hide', fig.cap="Coefficient of variation (CV) of the influence of the nearest feature and the cumulative influence of multiple features as the ZoI increases, for the different spatial configurations of point infrastructure. The ZoI was varied from 0.06\\% to 40\\% of the landscape extent (i.e., from 20 m to 12 km, given our landscapes are squares of 30 km extent). The y-axis is presented in log10 scale."}
cv_plot <- landscapes_dist %>% 
  ...
  labs(x = "Zone of Influence", 
       y = "Coefficient of variation of the influence",
...
# Influence measures in a real landscape

## Context: the Hardangervidda wild reindeer area

We also provide a real-world example to illustrate the similarities of $\phi_{nearest}$ and $\phi_{cum}$.
We use the Hardangervidda wild reindeer area in Southern Norway (Fig. 4 in the main text, Fig. C1), 
the region used for the habitat selection analyses presented in the main text. 
For more details, see the Appendix C. Similar to what was done above with the simulated landscapes, we calculated 
both $\phi_{nearest}$ and $\phi_{cum}$ for private cottages
and large tourist cabins for ZoI varying from 100 m to 20 km, which corresponds to ~0.1 to 20% of
the landscape extent (given the dimensions of the Hardangervidda reindeer area are at the order of 100 km).
We calculated $\phi_{nearest}$ and $\phi_{cum}$ using multiple possible influence functions:
linear decay, exponential decay, Gaussian decay, and threshold (see Fig. 1 of the main text). For each of them
we calculated the pixel-to-pixel correlation and the
CV, which are shown in Figs. B7 and B8.
...
The pattern of correlations among influence measures for private cottages follow the same
pattern predicted for clustered spatial points (Fig. B5), what is reasonable given their 
high density and aggregated spatial distribution 
(see Fig. C2 in Appendix C and Fig. 4 in the main text).
Since the minimum distance between private cottages is very small, even for the smallest 
values of ZoI $\phi_{nearest}$ and $\phi_{cum}$ are already different, and for all
influence functions their correlation
decreases with ZoI until ~ 5000 m (Fig. B7A), apart from the threshold function
(for which there is
no variation in $\phi_{nearest}$ for ZoI > 5000m; all value are 1; see Appendix C).
We did not calculate measures of or tested for clustering for these datasets, but the ZoI for which the correlation
stops decreasing for private cottages is related to the cluster size -- on relative terms, 
the distance between private cabins features is small (Table C1). 
In contrast, for public cabins the correlation among influence measures does not stop decreasing
with ZoI (Fig. B8A), which is also the expected from the predictions from our simulated landscapes for
random and sparse distributions of features (see the relative distance to the nearest tourist cabin
in Table C1). Here it would be necessary to go much beyond
ZoI = 20 km to detect some clustering in the spatial distribution of tourist cabins, a scale
in which the two $\phi$ could stop diverging. 

Looking at the variation in both measures for each infrastructure type,
we see for private cottages -- dense and aggregated -- $\phi_{cum}$ is more variable than
$\phi_{nearest}$ for most ZoI, what explains their divergence, 
and their correlation stops decreasing when the CV
for $\phi_{nearest}$ get next to zero (Fig. B7B). For tourist resorts -- 
sparse and spread in space -- the CV of both influence measures is similar for small
ZoI, and only increases when ZoI is large (Fig. B8B). 

```{r plot_corr_priv, eval=TRUE, echo=FALSE, results='hide', fig.asp=1.2, fig.cap="(A) Pixel-to-pixel correlation and (B) coefficient of variation of the influence of the nearest feature and the cumulative influence of multiple features as the ZoI increases, for private cottages in the Hardangervidda wild reindeer area. The vertical dashed line represents the minimum distance between features. The correlation between measures is not shown for the Threshold function with $ZoI \\geq 2500 m$, for which the CV of $\\phi_{nearest}$ is null."}
ggpubr::ggarrange(corr_plot_reindeer_priv, cv_plot_reindeer_priv, align = "v", nrow = 2, labels = "AUTO")
```


```{r plot_corr_pub, eval=TRUE, echo=FALSE, results='hide', fig.asp=1.2, fig.cap="(A) Pixel-to-pixel correlation and (B) coefficient of variation of the influence of the nearest feature and the cumulative influence of multiple features as the ZoI increases, for tourist cabins in the Hardangervidda wild reindeer area. The vertical dashed line represents the minimum distance between features."}
ggpubr::ggarrange(corr_plot_reindeer_pub, cv_plot_reindeer_pub, align = "v", nrow = 2, labels = "AUTO")
```

# Additional remarks

Here we showed that two measures of influence -- $\phi_{nearest}$ and $\phi_{cum}$ -- 
are more correlated either when the spatial distribution of features is sparse and the ZoI is small
(when the minimum distance between features is smaller than the $2 \cdot ZoI$) 
or when features are clustered and ZoI is large
(the ZoI is larger than the size of clusters of features). 
When the correlation between $\phi_{nearest}$ and $\phi_{cum}$ is high  
...
or the cumulative influence of multiple features (or none) remain an empirical question to be 
explored in real landscapes.

The results presented here  
assume infrastructure represented as points, chosen here because of the simplicity 
to represent them and place them following different patterns. Yet, these cases 
also provide insights on when $\phi_{nearest}$ and $\phi_{cum}$ are similar for other 
...
of when these influence measures converge might be needed.
