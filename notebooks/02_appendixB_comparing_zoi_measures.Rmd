...
abstract: | 
  ...
  it by simulating landscapes with different spatial configurations of infrastructure 
  ...
  to aid the interpretation of their effects on ecological contexts. This document 
  complements the arguments presented in Niebuhr et al. *Estimating the cumulative 
  impact and zone of influence of anthropogenic infrastructure on biodiversity.*
output: 
  NinaR::jensAnalysis:
    highlight: tango
    fig_caption: yes
...
selection analyses presented in the main text and in Appendix C.
...
ZoI radii. We see in Figures B2, B3, and B4 the two metrics for $r$ = 1.6%, 5%, 
...
(Fig. B2). As $r$ increases, however, going beyond the minimum distance between 
features, the ZoI from different features add up in the calculation of
$\phi_{cumulative}$ and the two ZoI metrics diverge (Fig. B3 and B4). 
...
As seen in the plots above (Figs. B2-B4), when the minimum distance between features
is larger than $2r$, the zone of influence of different features does not overlap 
and $\phi_{nearest}$ is similar to $\phi_{cumulative}$ (correlation = 1; Fig. B5). 
...
(= 10% of the landscape or 3000 m; Fig. B2 and B3, Fig. B5A), 
but less evident in the other spatial configuration of points, since the minimum distance between
points if very small (Fig. B5B-F). As $r$ increases, the two Zoi metrics diverge.
...
the two metrics diverge (Fig. B6).
...
```{r correlations, eval=TRUE, echo=FALSE, results='hide', fig.cap="Pixel-to-pixel correlation between the ZoI of the nearest feature and the cumulative ZoI of multiple features as the ZoI radius increases, for the different spatial configurations of point infrastructure. The ZoI radius was varied from 0.06\\% to 40\\% of the landscape extent (i.e., from 20 m to 12 km, given our landscapes are squares of 30 km extent)."}
# plot
...
```{r cv, eval=TRUE, echo=FALSE, results='hide', fig.cap="Coefficient of variation (CV) of the ZoI of the nearest feature and the cumulative ZoI of multiple features as the ZoI radius increases, for the different spatial configurations of point infrastructure. The ZoI radius was varied from 0.06\\% to 40\\% of the landscape extent (i.e., from 20 m to 12 km, given our landscapes are squares of 30 km extent). The y-axis is presented in log10 scale."}
...
area in Southern Norway (Fig. 5 in the main text, Fig. C1), 
the region used for the habitat selection analyses presented in the main text. 
For more details, see the Appendix C. Similar to what was done above with the 
simulated landscapes, we calculated both $\phi_{nearest}$ and $\phi_{cumulative}$ 
for private cottages and large tourist cabins for ZoI varying from 100 m to 20 km, 
...
(see Appendix A). For each of them we calculated the pixel-to-pixel correlation and the
...
(see Fig. C2 in Appendix C and Fig. 5 in the main text).
...
decreases with ZoI radius until $r \sim 5000$ m (Fig. B7A), apart from the threshold 
function (for which there is no variation in $\phi_{nearest}$ for $r > 5000$ m, since 
all the function values are 1; see Appendix C).
We did not calculate measures of or tested for clustering for these datasets, but 
the ZoI radius for which the correlation stops decreasing for private cottages 
is related to the cluster size -- on relative terms, the distance between private 
cabins features is small (Table C1). 
In contrast, for public cabins the correlation among the ZoI metrics does not 
stop decreasing with the ZoI radius (Fig. B8A), which is also the pattern expected from 
the predictions from our simulated landscapes for random and sparse distributions 
of features (see the relative distance to the nearest tourist cabin in Table C1). 
Here it would be necessary to go much beyond $r = 20$ km to detect some clustering 
in the spatial distribution of tourist cabins, a scale in which the two $\phi$ 
could stop diverging. 

Looking at the variation in both ZoI metrics for each infrastructure type,
we see for private cottages -- dense and aggregated -- $\phi_{cumulative}$ is more 
...
next to zero (Fig. B7B). For tourist resorts -- sparse and spread in space -- 
the CV of both influence measures is similar for small $r$, and increases with 
$r$ (Fig. B8B). 

```{r plot_corr_priv, eval=TRUE, echo=FALSE, results='hide', fig.asp=1.2, fig.cap="(A) Pixel-to-pixel correlation and (B) coefficient of variation of the ZoI of the nearest feature and the cumulative ZoI of multiple features as the ZoI radius increases, for private cottages in the Hardangervidda wild reindeer area. The vertical dashed line represents the minimum distance between features. The correlation between measures is not shown for the threshold function with $r \\geq 2500 m$, for which the CV of $\\phi_{nearest}$ is null."}
ggpubr::ggarrange(corr_plot_reindeer_priv, cv_plot_reindeer_priv, align = "v", nrow = 2, labels = "AUTO")
```


```{r plot_corr_pub, eval=TRUE, echo=FALSE, results='hide', fig.asp=1.2, fig.cap="(A) Pixel-to-pixel correlation and (B) coefficient of variation of the ZoI of the nearest feature and the cumulative ZoI of multiple features as the ZoI radius increases, for tourist cabins in the Hardangervidda wild reindeer area. The vertical dashed line represents the minimum distance between features."}
...
also provide insights on when $\phi_{nearest}$ and $\phi_{cumulative}$ are similar for other 
