...
abstract: |
  ...
  assess if and how the impacts of multiple infrastructure affect mountain reindeer 
  ...
  and zone of influence of anthropogenic infrastructure on biodiversity.*
...
obtained from GeoNorge map catalog (https://kartkatalog.geonorge.no/).
...
randomly within this wild reindeer area (Fig. C1). The combination of
...
assessed only the impacts of private cottages and large tourist cabins.
...
the function `glm` in R (R Core Team, 2021), with weight $w = 1$
...
cottage (Fig. C3A), what points to strong evidence that the impacts of multiple
private cottages on reindeer habitat selection accumulate. The
coefficients were consistently negative across ZoI radii (Fig. C3B),
...
ZoI radius varied from 500 m to 1000 m (Fig. C3A). Combining the results, we
...
to the combined impact of many individual cottages at a larger extent - a
...
the end of the day. Then, the ZoI radius of a single cottage is shorter.
...
influence (Fig. C4). The most supported ZoI variable was the
...
distance to the closest cabin (Fig. C2 and Table C1). As a
...
cottages), since the ZoI of each feature only start
...
distribution of features and small ZoI radius, $\phi_{nearest}$ and $\phi_{cum}$
...
cumulative ZoI of private cottages with threshold decay and $r = 10$ 
km and the cumulative ZoI of multiple tourist cabins with
exponential decay and $r = 20$ km ($\Delta AIC = 26.9$ from the 
second-ranked model, $wAIC = 1$; Table C2). Notice that, as
parameterized here, for the tourist cabins an exponential decay ZoI with radius
...
Table C2). This presents strong support for the cumulative impacts of both 
private cottages and tourist cabins on reindeer habitat selection in
Hardangervidda.
...
ZoI predictors), we see both infrastructure are avoided by
reindeer. Their impact vary differently across space since their
ZoI functions and radii differ -- a threshold ZoI with 10 km
radius for private cottages and an exponential decay ZoI with 20 km radius for
tourist cabins --, but also because the estimated effect size of a single
...
Table C3, Fig. C5A). However, since private cottages occur at much
higher densities, in some areas their overall impact is higher
than that of tourist cabins (Fig. C5, Fig. 4 in the main text). 
Comparing an area with only 1 cottage in a 10 km radius with an area with 
only 1 tourist cabins in a 
...
for private cottages (Fig. C5A). In contrast, if we take the areas with
...
than that of tourist cabins (Fig. C5C). Following
...
for(i in 1:length(infra)) {
  df_plots[[i]] <- plot_zoi1d(points, zoi_est[i], cumulative = cum[i],
                              fun = func[i], 
...
```{r beta_and_zoi_plot, echo=FALSE, eval=TRUE, fig.cap='Impact of private cottages and tourist cabins considering (A) only 1 feature, (B) the median number of features (103 for private cottages, 0.38 for tourist cabins), and (C) the maximum number of each type of feature (2664 for cottages, 5 for cabins), given their respective estimated ZoI shape and radius The impact presented here is the multiplication between the effect size (the model coefficients) and the cumulative ZoI variable (eq. 5 in the main text). The impact of only one private cottage is negligible (A). At their median values, the impacts of private and public cabins are comparable (B), while at their maximum the cumulative impact of private cottages might be higher than that of tourist cabins (C).'}
g6
```
...
tm_basemap("OpenTopoMap") +
  ...
  tm_shape(raster_nearest$public_cabins_high_inf_nearest_exp_decay20000,
           name = "NearZoI Public cabins high 20km") +
  tm_raster(style = "cont") +
  ...
  tm_shape(rasters$public_cabins_high_cumulative_exp_decay_20000,
           name = "CumZoI Public cabins high 20km") +
  tm_raster(style = "cont") +
...
p_priv_cab_cuminf <- tm_shape(rasters[[1]]) +
  ...
  tm_shape(private_cabins_sf, name = "Private cottages") +
  tm_dots() +
  tm_layout(main.title = "Cumulative zone of influence - Private cottages", 
            legend.frame = T,
            main.title.size = 1, legend.title.size = 1)
p_priv_cab_cuminf
...
p_pub_cab_cuminf <- tm_shape(rasters[[2]]) +
  ...
  tm_shape(public_cabins_high_sf, name = "Tourist cabins") +
  tm_dots() +
  tm_layout(main.title = "Cumulative zone of influence - Tourist cabins",
            legend.frame = T,
            main.title.size = 1, legend.title.size = 1)
p_pub_cab_cuminf
...
p_priv_cab_nearinf <- tm_shape(raster_nearest[[1]]) +
  ...
  tm_shape(private_cabins_sf, name = "Private cottages") +
  tm_dots() +
  tm_layout(main.title = "Nearest zone of influence - Private cottages",
            legend.frame = T,
            main.title.size = 1, legend.title.size = 1)
p_priv_cab_nearinf
...
p_pub_cab_nearinf <- tm_shape(raster_nearest[[2]]) +
  ...
  tm_shape(public_cabins_high_sf, name = "Tourist cabins") +
  tm_dots() +
  tm_layout(main.title = "Nearest zone of influence - Tourist cabins",
            legend.frame = T,
            main.title.size = 1, legend.title.size = 1)
p_pub_cab_nearinf
