---
title: "Appendix D"
subtitle: "Getting started with the `oneimpact` package in R"
author: ""
date: ""
geometry: margin=2cm
abstract: |
  In this document we introduce the `oneimpact` package in R. `oneimpact` is a
  collection of tools and functions that aim at aiding the estimation of cumulative
  impacts in ecological studies and environmental impact assessments. This 
  document shows the R implementation of the approach presented in the main 
  text of Niebuhr et al. *Estimating the cumulative impact 
  and zone of influence of anthropogenic infrastructure on biodiversity.* It
  presents an overview of the package functions and simple examples on how to use
  them.
output:
  NinaR::jensAnalysis:
    highlight: tango
...
Here we give an overview of the functions in `oneimpact`, define and provide 
examples for the zone of influence functions, show how to use the main functions, 
and provide an example of how to use this approach to annotate data for 
statistical analysis.

We start by loading the `oneimpact` package and other packages used in this
vignette.
...
in R are fast and easy-to-use, since they make use of the optimized functions from the
`terra` package (Hijmans, 2022). However, computations might become slow for large maps. In GRASS GIS,
on the other hand, it is possible to compute the ZoI for very large maps using the
...
| Create filters | `save_filter()` | This function saves a matrix with weights (filter or kernel matrix) in an external text file. It can save either the raw matrix or use the standards for running `r.mfilter` within GRASS GIS. |
...
is based on neighborhood analysis using spatial filters that determine the ZoI
...
of the modules in GRASS GIS: `r.mfilter`, `r.resamp.filter`, and `r.neighbors`. 
...
### Functions with a well-defined radius

Some functions vanish for a certain non-infinite distance and therefore present
well-defined radii. Here the ZoI radius $r$ represents the distance at which $\phi = 0$.
...
the ZoI radius as $\verb|zoi_radius = 3|$. We start by plotting the ZoI of the nearest
feature only by setting $\verb|cumulative = FALSE|$:
...
Now we do the same but considering the ZoI of each feature accumulates. We do it by
...
The data is mapped as points in vector format, and more information about it might be
...
by lines or polygons (e.g. roads, power lines, deforestation areas), it is enough to
...
`grass_v2rast_count()` to count the number of features per pixel and get the output as
a raster object.
...
but in GRASS GIS this is not done. In this case, the easiest is to prepared the input
...
For `calc_zoi_cumulative()` The user might choose between the cumulative ZoI metric when
...
# Using the approach to annotate and analyze data
...
infrastructure features accumulate and the identification of the ZoI shape and size 
...
knitr::include_graphics("../text/figures/figure_workflow.png")
```
...
Here we create a dataset of sampling points and a theoretical random response 
variable to show the process of calculation of ZoI variables and the annotation
...
the locations using the `set_points()` function from the `oneimpact` package and 
...
different types of infrastructure through statistical models. Statistical analyses 
can make use of model selection (Burnham & Anderson, 2002; Huais, 2018), 
