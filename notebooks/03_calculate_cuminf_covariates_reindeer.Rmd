...
nearinf_euc <- purrr::map(infrastructure, oneimpact::calc_influence_nearest,
                          type = "euclidean", where = "GRASS",
                          quiet = TRUE, overwrite = TRUE)
nearinf_euc <- unlist(nearinf_euc)

# cut and retrieve maps for the study area
layers_in_r <- list()
for(i in 1:length(nearinf_euc)) {
  # retrieve to R
  layers_in_r[[i]] <- rgrass7::read_RAST(nearinf_euc[i], return_format = "terra")
  #export from R
  terra::writeRaster(layers_in_r[[i]], 
                     filename = paste0("../data/analysis_GPS/", nearinf_euc[i], ".tif"), 
                     overwrite = TRUE)
  # export from GRASS
  # rgrass7::execGRASS("v.out.ogr", input = vect_export_within[i], 
  #                    output = paste0("../data/analysis_GPS/", out_names[i]))
